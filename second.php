<?php
// Наш текст
$My_text = "Нет никого, кто любил бы боль Саму по себе, кто искал бы её и кто хотел бы иметь её просто потому, что это боль.";
...
function znak($text) {
    ...
    foreach ($value_revers as $leter) {
        ...
        if ($leter == "," || $leter == ";" || $leter == ":" || $leter == "-" || $leter == "." || $leter == "!" || $leter == "?") {
            //Нашли знак
            $znak = 1; // Ставим флажок что знак есть
            $znak_L = $leter;
        }
    }
    // ЕСЛИ знак есть
